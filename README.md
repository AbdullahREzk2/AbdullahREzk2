<h1 align="center">👋 Hi, I'm Abdullah Rezk</h1>
<h3 align="center">Software Developer | Backend .NET Developer</h3>

---

### 🧾 About Me
I’m a **Computer and Information Science graduate** from **Mansoura University**, majoring in **Computer Science**.  
I specialize in **backend development** using **ASP.NET Core Web API**, and I’m passionate about building clean, scalable, and efficient software systems.  
I focus on transforming real-world problems into robust digital solutions that make an impact.

---

### ⚙️ Technical Skills
**Languages & Core Concepts:**  
C#, Object-Oriented Programming (OOP)  

**Frameworks & Technologies:**  
ASP.NET Core Web API, Entity Framework, ADO.NET, LINQ  

**Databases & Tools:**  
SQL Server, Postman, Git  

---

### 🚀 Featured Projects

#### 🧠 SmartBite — Graduation Project
A **nutrition assistant** that helps users organize and manage their meals based on their health conditions or personal goals.  
Built with **Flutter** (frontend) and **ASP.NET Core Web API** (backend) using **ADO.NET** for database operations.  
📎 **Repository:** [SmartBite Repository](https://github.com/AbdullahREzk2/Smart_Bite)  
📄 **Description:** Integrates intelligent meal recommendation logic and supports user goal tracking.

---

#### 🛍️ Click To Buy — E-commerce Website
An **e-commerce platform** for a local clothing brand, allowing users to browse, search, and purchase apparel online.  
Developed with **React.js** for the frontend and **ASP.NET Core Web API** using **Entity Framework** for backend operations.  
📎 **Repository:** [Click To Buy Repository](https://github.com/AbdullahREzk2/E-Commerce-API)  
🌐 **Live Website:** [Click To Buy Website](https://e-commerce-iti-six.vercel.app/)  
📄 **Description:** Features secure authentication, product management, and a responsive user interface.

---

#### 🏥 Medical Center — Desktop Application
A **Windows Forms desktop application** for managing medical appointments, patients, and doctors.  
Connected to an **ASP.NET Core Web API** backend with **Entity Framework** for data management.  
📎 **Repository:** [Medical Center Repository](https://github.com/AbdullahREzk2/Medical-Center)  
📄 **Description:** Enables efficient appointment scheduling and patient record handling for clinics and small medical centers.

---

### 📫 Contact
- **LinkedIn:** [linkedin.com/in/abdullahrezk03](https://www.linkedin.com/in/abdullahrezk03/)  
- **Email:** [abdullahelkashlan@gmail.com](mailto:abdullahelkashlan@gmail.com)



---

### 🧩 Quote
> “Code is like humor. When you have to explain it, it’s bad.” — Cory House
